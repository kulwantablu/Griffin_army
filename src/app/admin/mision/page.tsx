"use client";

import { useEffect, useState } from "react";
import { useRouter } from "next/navigation";

export default function EditAbout() {
    const [mission, setMission] = useState({
        heading: '',
        detail: ''
      });
  const router = useRouter();

  useEffect(() => {
    fetch(`/api/mission/get`)
      .then((res) => res.json())
      .then((data) => {
        console.log("Fetched data:", data);
        setMission(data); 
      });
  }, []);
  

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();

    console.log("Submitting data:", mission);

    const response = await fetch("/api/mission/edit", {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({
        heading: mission.heading,
        detail: mission.detail,
      }),
    });

    const data = await response.json();

    if (response.ok && data.message === "mission updated successfully") {
      alert("mission updated!");
      router.push("/admin/mision");
    } else {
      alert("Failed to update: " + data.message);
      console.error("Error:", data);
    }
  };

  return (
    <div style={{ padding: "20px", width: "60%", margin: "auto" }}>
      <div className="text-center">
        <h1 className="h4 text-gray-900 mb-4">mission</h1>
      </div>
      <form onSubmit={handleSubmit}>
        <div className="form-group row">
          <div className="col-sm-12">
            <label>Heading</label>
            <input
              type="text"
              className="form-control form-control-user"
              value={mission.heading}
              onChange={(e) => setMission({ ...mission, heading: e.target.value })}
              required
            />
          </div>
        </div>

        <div className="form-group row">
          <div className="col-sm-12">
            <label>Detail</label>
            <textarea
              className="form-control form-control-user"
              value={mission.detail}
              onChange={(e) => setMission({ ...mission, detail: e.target.value })}
              required
            />
          </div>
        </div>

        <div className="form-group row">
          <div className="col-sm-12 mb-3 mb-sm-0">
            <button type="submit" className="btn btn-primary btn-user">
              Update
            </button>
          </div>
        </div>
      </form>
    </div>
  );
}
